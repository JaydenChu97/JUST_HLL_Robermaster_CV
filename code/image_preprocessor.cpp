                                      ...
                                      Mat& Value)
{
    ...
    for(int a=0; a<contours.size()-1; a++)
    {
        ...
        for (int b = a+1; b<contours.size(); b++)
        {
            ...
            double a1 = boundRect[a].x,
                   a2 = boundRect[a].x + boundRect[a].width,
                   a3 = boundRect[b].x,
                   a4 = boundRect[b].x + boundRect[b].width;
            double b1 = boundRect[a].y,
                   b2 = boundRect[a].y + boundRect[a].height,
                   b3 = boundRect[b].y,
                   b4 = boundRect[b].y + boundRect[b].height;
            ...
            if (abs(center[b].y - center[a].y) < abs(center[b].x - center[a].x)*0.8)//夹角
            {
                for(int i=top; i<bottom; i++)
                {
                    ...
                    uchar* staturationData = Staturation.ptr<uchar>(i);
                    for(int j=left; j<right; j++)
                    {
                        if(hueData[j] == 255)
                            redpixel[0]++;
                        if(staturationData[j] == 255)
                            dark[0]++;
                    }
                }
                ...
                {
                    if(boundRect[a].height > 0.5*boundRect[a].width
                            &&boundRect[b].height > 0.5*boundRect[b].width)
                    {
